...
let surveyURL = 'https://vku.udn.vn/';

// Redirect on leave room
let redirectActive = false;
let redirectURL = 'https://vku.udn.vn/';
...
function genGravatar(email, size = false) {
    ...
    const gravatarURL = `${hash}` + (size ? `?s=${size}` : '?s=250');
    return gravatarURL;
    function md5(input) {
...
function setMyWhiteboardBtn() {
    ...
    whiteboardGhostButton.addEventListener('click', (e) => {
        ...
        setWhiteboardBgColor(wbIsBgTransparent ? '#008080' : wbBackgroundColorEl.value);
...
function shareRoomMeetingURL(checkScreen = false) {
    ...
    Swal.fire({
        ...
        <br/>
        <p style="color:rgb(8, 189, 89);">Join room by this link</p>
